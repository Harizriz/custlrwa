    ...
    while True:
        try:
            ...
            for i in range(len(jsonObj['products'])):
                ...
                cursor.execute(sql_insert_product, (product_id, product_name, price, currency, 'H&M', original_site, category, image_url))
                connection.commit()

                for img in productArticleDetails[jsonObj['products'][i]['articleCode']]['images']:
                    cursor.execute(sql_insert_image,('https:' + img['zoom'], product_id))
                    connection.commit()
                ...
                for size in productArticleDetails[jsonObj['products'][i]['articleCode']]['sizes']:
                    if any(size['sizeCode'] in code for code in availability):
                        cursor.execute(sql_insert_size, (size['name'], product_id))
                        connection.commit()
